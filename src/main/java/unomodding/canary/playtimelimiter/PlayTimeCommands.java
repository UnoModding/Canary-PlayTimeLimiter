...
public class PlayTimeCommands implements CommandListener {
    ...
    @Command(aliases = { "playtime", "pt" }, description = "playtime command", permissions = {}, toolTip = "/playtime <start|stop|add|remove|check> [parameters...]", version = 2)
        ...
        if (args.length != 0) {
            if (args[0].equals("start") && args.length == 1) {
                ...
                if (!caller.hasPermission("playtimelimiter.playtime.stop")) {
                    caller.message(TextFormat.RED + "You don't have permission to stop the playtime counter!");
                } else {
                    if (!plugin.stop()) {
                ...
                if (!caller.hasPermission("playtimelimiter.playtime.remove")) {
                    ...
                    try {
                        plugin.addPlayTime(Canary.getServer().getPlayer(args[1]).getUUIDString(),
                ...
                if (!caller.hasPermission("playtimelimiter.playtime.add")) {
                    ...
                    try {
                        plugin.removePlayTime(Canary.getServer().getPlayer(args[1]).getUUIDString(),
                                Integer.parseInt(args[2]));
                ...
                if (args.length == 1) {
                    if (!caller.hasPermission("playtimelimiter.playtime.check.self")) {
                        ...
                        caller.message(TextFormat.GREEN
                                + "You have played for "
                                + plugin.secondsToDaysHoursSecondsString(plugin.getPlayerPlayTime(Canary
                                        .getServer().getPlayer(caller.getName()).getUUIDString()))
                                + " and have "
                                + plugin.secondsToDaysHoursSecondsString(plugin
                                        .getTimeAllowedInSeconds(Canary.getServer()
                                                .getPlayer(caller.getName()).getUUIDString()
                                                + " remaining!")));
                    }
                    return;
                    ...
                    if (!caller.hasPermission("playtimelimiter.playtime.check.others")) {
                        ...
                        caller.message(TextFormat.GREEN
                                ...
                                + " has played for "
                                + plugin.secondsToDaysHoursSecondsString(plugin.getPlayerPlayTime(Canary
                                        .getServer().getPlayer(args[1]).getUUIDString()))
                                + " and has "
                                + plugin.secondsToDaysHoursSecondsString(plugin
                                        .getTimeAllowedInSeconds(Canary.getServer().getPlayer(args[1])
    ...
    public void printUsage(MessageReceiver sender) {
        ...
        if (sender.hasPermission("playtimelimiter.playtime.check.others")) {
            sender.message(TextFormat.CYAN + "/playtime check [user]" + TextFormat.RESET
    ...
    public List<String> playtimeTabComplete(MessageReceiver caller, String[] parameters) {
        if (parameters.length == 1) {
            return TabCompleteHelper.matchTo(parameters, new String[] { "start", "stop", "add", "remove",
                    "check" });
        } else if (parameters.length == 2
                && (parameters[1].equals("add") || parameters[1].equals("remove") || parameters[1]
                        .equals("check"))) {
